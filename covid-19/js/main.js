function init(count) {
  ...
  document.body.innerHTML += `<div class="player" style="width: ${playerSize}px; height: ${playerSize}px; top: 70px; left: 70px; background: url('mediafiles/svg/covid.svg') no-repeat center / cover;"></div>`;

  for(let i = 0; i < count; i++) {
    let size = randomNum(minSize, maxSize);
    document.body.innerHTML += `<div class="enemy" style="top: ${randomNum(minPos, maxPos)}vh; left: ${randomNum(minPos, maxPos)}vw; width: ${size}px; height: ${size}px; background: url('mediafiles/svg/virus${randomNum(1, 4)}.svg') no-repeat center / cover;"></div>`;
  }
}
